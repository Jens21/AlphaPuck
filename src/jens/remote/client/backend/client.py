...
'''
    ...
    def connection_error(self,
        ...
        if conn_err == NetworkInterfaceConnectionError.CONNECTING:
            print('Could not connect to server. Please try again later or contact the administrator', flush=True)
        elif conn_err == NetworkInterfaceConnectionError.LOST:
            print('Connection to Server lost. Please try again later or contact the administrator', flush=True)
    ...
    def game_done(self,
        ...
        if self.verbose:
                print(f"Winner: {info['winner']}", flush=True)
                print(f'{result["games_played"]} games played. You won {result["games_won"]} games. You lost {result["games_lost"]} games. {result["games_drawn"]} game(s) end in a draw.', flush=True)
        self.current_game.add_transition(next_obs=ob,
                                         next_action=None,
                                         r=r,
                                         done=done,
